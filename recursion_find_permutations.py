...
def find_permutations(string):
    ...
    if len(string) == 1:
        ...
        for i, letter in enumerate(string):
            ...
            # string[:1] means go till everything upto index i and string[i+1:0] means go through all the way till the end from index i+1
            for permutation in find_permutations(string[:i] + string[i+1:]):
